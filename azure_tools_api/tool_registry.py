...
if TYPE_CHECKING:
    ...
    from app import models, schemas as core_schemas  # type: ignore
...
class ToolSpec:
    ...
    # Use a permissive handler type to avoid requiring SQLAlchemy types at import time.
...
_TOOL_SPECS_CACHE: dict[str, ToolSpec] | None = None


def _build_tool_specs() -> dict[str, ToolSpec]:
    """Build tool specs lazily to avoid importing application schemas at module
    import time. This imports `app.schemas` only when the tool specs are
    requested.
    """
    ...
    from app import schemas as core_schemas  # local import to avoid top-level import
